...
public partial class Emit<TDelegateType>
{
    private void FailStackUnderflow(int expected, string method)
    {
        if (expected == 1)
        {
            throw new SigilVerificationException(method + " expects a value on the stack, but it was empty", _il.Instructions(_allLocals));
        }

        throw new SigilVerificationException(method + " expects " + expected + " values on the stack", _il.Instructions(_allLocals));
    }
    ...
    private void ValidateTryCatchFinallyBranches()
    {
        ...
        {
            var instr = _branchPatches[branch.Item3];

            var toLabel = branch.Item2;
            var fromIndex = branch.Item3;

            var toIndex = _marks[toLabel];

            var fromTryBlocks = _tryBlocks.Where(t => fromIndex >= t.Value.Item1 && fromIndex <= t.Value.Item2).ToList();
            var fromCatchBlocks = _catchBlocks.Where(c => fromIndex >= c.Value.Item1 && fromIndex <= c.Value.Item2).ToList();
            var fromFinallyBlocks = _finallyBlocks.Where(f => fromIndex >= f.Value.Item1 && fromIndex <= f.Value.Item2).ToList();

            var toTryBlocks = _tryBlocks.Where(t => toIndex >= t.Value.Item1 && toIndex <= t.Value.Item2).ToList();
            var toCatchBlocks = _catchBlocks.Where(c => toIndex >= c.Value.Item1 && toIndex <= c.Value.Item2).ToList();
            var toFinallyBlocks = _finallyBlocks.Where(f => toIndex >= f.Value.Item1 && toIndex <= f.Value.Item2).ToList();
    ...
    private void Validate()
    {
        ...
        var tracer = new ReturnTracer(_branches, _marks, _returns, _throws);

        var result = tracer.Verify();
        if (!result.IsSuccess)
        {
            throw new SigilVerificationException("All execution paths must end with Return", result, _il.Instructions(_allLocals));
        }
