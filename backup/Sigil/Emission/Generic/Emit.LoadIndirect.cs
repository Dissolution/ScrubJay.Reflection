...
public partial class Emit<TDelegateType>
{
    ...
    /// </summary>
    public Emit<TDelegateType> LoadIndirect<T>(bool isVolatile = false, int? unaligned = null)
    {
        return LoadIndirect(typeof(T), isVolatile, unaligned);
    }

    /// <summary>
    /// Pops a pointer from the stack and pushes the value (of the given type) at that address onto the stack.
    /// </summary>
    public Emit<TDelegateType> LoadIndirect(Type type, bool isVolatile = false, int? unaligned = null)
    {
        if (type == null)
        {
