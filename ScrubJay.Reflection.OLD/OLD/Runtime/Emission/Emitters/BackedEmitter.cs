...
using ScrubJay.Reflection.OLD.OLD.Runtime.Emission.Instructions;

namespace ScrubJay.Reflection.OLD.OLD.Runtime.Emission.Emitters;

internal class BackedEmitter<TEmitter> : Emitter<TEmitter>
    where TEmitter : IILEmitter<TEmitter> //ICleanEmitter<TEmitter>, IDirectEmitter<TEmitter>, ISimpleEmitter<TEmitter>
{
    ...
    public BackedEmitter(ILGenerator ilGenerator) : base()
    {
    ...
    protected override EmitterLabel CreateEmitterLabel(string? labelName)
    {
...
#region IGeneratorEmitter<TEmitter>
    public override TEmitter BeginExceptionBlock(out EmitterLabel label, string? labelName = null)
    {
        var lbl = _ilGenerator.BeginExceptionBlock();
        label = new EmitterLabel(lbl, labelName);
    ...
    {
        _ilGenerator.EmitCall(Extensions.GetCallOpCode((MethodBase)methodInfo), methodInfo, optionalParameterTypes);
