...
public partial class Emit<TDelegateType>
{
    ...
    {
        while(_readonlyPatches.Count > 0)
        {
            var elem = _readonlyPatches[0];
            _readonlyPatches.RemoveAt(0);
    ...
    /// </summary>
    public Emit<TDelegateType> LoadElementAddress<TElementType>()
    {
        return LoadElementAddress(typeof(TElementType));
    }
    ...
    /// Pops both, and pushes the address of the element at the given index.
    /// </summary>
    public Emit<TDelegateType> LoadElementAddress(Type elementType)
    {
        ...
        _readonlyPatches.Add(Tuple.Create(_il.Index, pushToStack));
