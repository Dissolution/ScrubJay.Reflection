...
{
    private readonly Emit<NonGenericPlaceholderDelegate> _innerEmit;
    private Module _module;
    private string _name;
    private Type _returnType;
    private Type[] _parameterTypes;

    private Delegate _createdDelegate;
    private MethodBuilder _createdMethod;
    private ConstructorBuilder _createdConstructor;

    private readonly NonGenericEmitType _emitType;

    private TypeBuilder _typeBuilder;
    private MethodAttributes _attributes;
    private CallingConventions _callingConvention;

    private readonly EmitShorthand _shorthand;
    ...
    private Emit(Emit<NonGenericPlaceholderDelegate> innerEmit, NonGenericEmitType type)
    {
        ...
        _shorthand = new EmitShorthand(this);
    }
    ...
    public static Emit NewDynamicMethod(Type returnType, Type[] parameterTypes, string name = null, ModuleBuilder module = null, bool doVerify = true, bool strictBranchVerification = false)
    {
        ...
        module = module ?? Emit<NonGenericPlaceholderDelegate>._module;
        ...
        ret._module = module;
        ret._name = name ?? AutoNamer.Next("_DynamicMethod");
        ret._returnType = returnType;
        ret._parameterTypes = parameterTypes;
    ...
    private void ValidateDelegateType(Type delegateType)
    {
        ...
        {
            throw new ArgumentException("Expected delegateType to return " + _returnType + ", found " + returnType);
        }
        ...
        {
            throw new ArgumentException("Expected delegateType to take " + _parameterTypes.Length + " parameters, found " + parameterTypes.Length);
        }

        for (var i = 0; i < parameterTypes.Length; i++)
        {
            var actualType = parameterTypes[i].ParameterType;
            var expectedType = _parameterTypes[i];
    ...
    {
        if (_emitType != NonGenericEmitType.DynamicMethod)
        {
        ...
        if (_innerEmit.DynMethod == null)
        {
            var dynMethod = new DynamicMethod(_name, _returnType, _parameterTypes, _module, skipVisibility: true);

            _innerEmit.DynMethod = dynMethod;
        }
        ...
        _createdDelegate = _innerEmit.InnerCreateDelegate(delegateType, out instructions, optimizationOptions);
    ...
    /// behaves unexpectedly (indicative of a logic bug in the consumer code).
    /// </summary>
    public TDelegateType CreateDelegate<TDelegateType>(out string instructions, OptimizationOptions optimizationOptions = OptimizationOptions.All)
    {
        return (TDelegateType)(object)CreateDelegate(typeof(TDelegateType), out instructions, optimizationOptions);
    }
    ...
    {
        string ignored;
        return CreateDelegate<TDelegateType>(out ignored, optimizationOptions);
    }
    ...
    public static Emit BuildMethod(Type returnType, Type[] parameterTypes, TypeBuilder type, string name, MethodAttributes attributes, CallingConventions callingConvention, bool allowUnverifiableCode = false, bool doVerify = true, bool strictBranchVerification = false)
    {
        ...
        ret._name = name;
        ret._returnType = returnType;
        ret._parameterTypes = passedParameterTypes;
        ret._attributes = attributes;
        ret._callingConvention = callingConvention;
        ret._typeBuilder = type;
    ...
    {
        if (_emitType != NonGenericEmitType.Method)
        {
            throw new InvalidOperationException("Emit was not created to build a method, thus CreateMethod cannot be called");
        }

        if (_createdMethod != null)
        {
            instructions = null;
            return _createdMethod;
        }

        var methodBuilder = _typeBuilder.DefineMethod(_name, _attributes, _callingConvention, _returnType, _parameterTypes);

        _innerEmit.MtdBuilder = methodBuilder;

        _createdMethod = _innerEmit.CreateMethod(out instructions, optimizationOptions);
...
/*#if NETSTANDARD
        ...
        ret._returnType = type;
        ret._parameterTypes = passedParameters;
        ret._attributes = attributes;
        ret._callingConvention = callingConvention;
        ret._typeBuilder = type;
    ...
    public static Emit BuildTypeInitializer(TypeBuilder type, bool allowUnverifiableCode = false, bool doVerify = true, bool strictBranchVerification = false)
    {
        ...
        var innerEmit = Emit<NonGenericPlaceholderDelegate>.MakeNonGenericEmit(CallingConventions.Standard, typeof(void), [], allowUnverifiableCode, doVerify, strictBranchVerification);
        innerEmit.ConstructorDefinedInType = type;
        ...
        ret._parameterTypes = [];
        ret._attributes = 0;
        ret._callingConvention = CallingConventions.Standard;
        ret._typeBuilder = type;
    ...
    {
        if (_emitType != NonGenericEmitType.Constructor)
        {
            throw new InvalidOperationException("Emit was not created to build a constructor, thus CreateConstructor cannot be called");
        }

        if (_createdConstructor != null)
        {
            instructions = null;
            return _createdConstructor;
        }

        var constructorBuilder = _typeBuilder.DefineConstructor(_attributes, _callingConvention, _parameterTypes);

        _innerEmit.ConstrBuilder = constructorBuilder;

        _createdConstructor = _innerEmit.CreateConstructor(out instructions, optimizationOptions);
    ...
    {
        if (_emitType != NonGenericEmitType.TypeInitializer) {
        ...
        var constructorBuilder = _typeBuilder.DefineTypeInitializer();

        _innerEmit.ConstrBuilder = constructorBuilder;

        _createdConstructor = _innerEmit.CreateTypeInitializer(out instructions, optimizationOptions);
