...
public interface IFluentDirectOperationEmitter<TEmitter> : IFluentILEmitter<TEmitter>
...
/* Unmerged change from project 'ScrubJay.Reflection (net8.0)'
Before:
/// <summary>
...
After:
    /// <summary>
    /// Loads the element from an array index onto the stack as a <see cref="nint"/>.
    /// </summary>
    /// <exception cref="NullReferenceException">If the <see cref="Array"/> on the stack is <see langword="null"/>.</exception>
    /// <exception cref="IndexOutOfRangeException">If the index on the stack is negative or larger than the upper bound of the <see cref="Array"/>.</exception>
    /// <exception cref="ArrayTypeMismatchException">If the <see cref="Array"/> does not hold <see cref="nint"/> elements.</exception>
    /// <see href="http://docs.microsoft.com/en-us/dotnet/api/system.reflection.emit.opcodes.ldelem_i"/>
    TEmitter Ldelem_I();
*/
    /// <summary>
...
After:
    /// <summary>
    /// Replaces the <see cref="Array"/> element at a given index with the <see cref="nint"/> value on the stack.
    /// </summary>
    /// <exception cref="NullReferenceException">If the <see cref="Array"/> on the stack is <see langword="null"/>.</exception>
    /// <exception cref="IndexOutOfRangeException">If the index on the stack is negative or larger than the upper bound of the <see cref="Array"/>.</exception>
    /// <exception cref="ArrayTypeMismatchException">If the <see cref="Array"/> does not hold <see cref="nint"/> elements.</exception>
...
After:
    #region Ldind
    /// <summary>
    /// Loads a <see cref="nint"/> value from an address onto the stack.
    /// </summary>
    /// <exception cref="NullReferenceException">If an invalid address is detected.</exception>
    /// <see href="http://docs.microsoft.com/en-us/dotnet/api/system.reflection.emit.opcodes.ldind_i"/>
    TEmitter Ldind_I();
...
After:
    #region Stind
    /// <summary>
    /// Stores a <see cref="nint"/> value in a supplied address.
    /// </summary>
    /// <exception cref="NullReferenceException">If an invalid address is detected.</exception>
    /// <see href="http://docs.microsoft.com/en-us/dotnet/api/system.reflection.emit.opcodes.stind_i"/>
    TEmitter Stind_I();
